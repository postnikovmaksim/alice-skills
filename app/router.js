...
async function createBillAction({ sessionContext }) {
    ...
    if (!kontragentName) Promise.resolve({ message: 'К сожалению не удалосьпонять имя контрагента, попробуйте еще раз' });

    const kontragents = await kontragentService.getKontragent({ name: kontragentName }).catch(e => e);
    if (kontragents.length === 0) Promise.resolve({ message: 'Контрагент с таким именем не найден, попробуйте еще раз' });
    if (kontragents.length > 1) Promise.resolve({ message: 'К сожалению не полуться создать счет, так как контрагентов с таким именем больше одного. Могу ли я чем-то еще помочь?' });

    const productName = await identifyService.identifyProduct({ command }).catch(e => e);
    if (!productName) Promise.resolve({ message: 'Не удалось определить название товара, попробуйте еще раз' });
    const products = await productService.getProduct({ name: productName }).catch(e => e);
    if (products.length === 0) Promise.resolve({ message: 'Товар с таким названием не найден, попробуйте еще раз' });
    if (products.length > 1) Promise.resolve({ message: 'К сожалению не полуться создать счет, так как товаров с таким названием больше одного. Могу ли я чем-то еще помочь?' });
